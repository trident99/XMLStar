...
int XMLReader::LoadXMLTree(XMLElement * ptrCurrTop,const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	//find the starting index for the element
	int intReturn;
	//now that we found the starting position, hand it off to the XMLReader
	//Clear();
	intReturn = LoadXMLElement(ptrCurrTop, strInput, objCurrElementIndex);
	return intReturn;
};

int XMLReader::LoadXMLTree(XMLElement * ptrCurrTop, std::istream & strStreamInput)
{
	...
	XMLElementIndex objCurrElementIndex;
	intTemp = LoadXMLTree(ptrCurrTop,strFileInput,objCurrElementIndex);
	if(intTemp < 0){intReturn = -1;};

	return intReturn;
};
int XMLReader::LoadXMLTree(XMLElement * ptrCurrTop, std::ifstream & strStreamInput)
{
	int intTemp, intReturn;
	std::string strFileInput;
	intReturn = 1;
...
int XMLReader::LoadXMLElement(XMLElement * ptrElement, const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{	
	/*
	//READELEMENT
...
int XMLReader::LoadOpenTag(XMLElement * ptrElement, const std::string & strInput, XMLElementIndex & objCurrElementIndex, bool & blnNullElementReturn)
{
	int intTemp;
	size_t intCursor, intPosTag, intOpenPos, intClosePos;
	std::string strTagName;
	XMLNodeType enumNodeType = NODE;
	...
	while(intCursor < strInput.size())
	{
		...
		{//then valid open tag. check special cases first
			...
			if(IsNodeComment(strInput,intOpenPos))
			{
				...
				XMLElementIndex objCommentIndex;
				...
				XMLElementIndex objProcessIndex;
...
//Load the Tag to the Element Name
int XMLReader::LoadCloseTag(XMLElement * ptrElement, const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	return -1;
};

//Load the Attributes into the Element
int	XMLReader::LoadAttributes(XMLElement * ptrElement, const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	...
	XMLAttribute * ptrCurrAttrib = NULL;
	...
	while(intCursor < objCurrElementIndex.m_intOpenTagEndPos)
	{
		...
		{//then valid, continue extracting name
			...
			//clean the string of white space
			CleanNameString(objNVPTemp.first);
...
//Load the Attributes into the Element
int	XMLReader::LoadValue(XMLElement * ptrElement, const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	//makes the assumption that the open tag has been established.
	//take the tag string minus open and close characters
	//Parse the attributes into a name,value pair vector stripping the "" or '' around the value and the = sign
...
//Load the Attributes into the Element
int	XMLReader::LoadSubNodes(XMLElement * ptrElement, const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	//makes the assumption that the open tag has been established.
	//take the tag string minus open and close characters
	//Parse the attributes into a name,value pair vector stripping the "" or '' around the value and the = sign
	...
	while((!blnCloseTagFound)&&(intCursor < lngStringSize))
	{
		...
		XMLNodeType enumCurrNodeType = NODE;
		//find next open tag
		...
		{//then check to see what type of tag this is.
			...
			if((blnIsElement)&&(enumCurrNodeType == NODE))
			{
				...
				XMLElementIndex objSubElementIndex;
...
int	XMLReader::LoadSubElement(XMLElement * ptrElement, const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	...
	intReturn = 1;
	XMLElement * ptrNewSubElement = ptrElement->AddSubElement();
	if(ptrNewSubElement)
	{//then valid sub element creation, call that elements Loading function.
		intTemp = LoadXMLElement(ptrNewSubElement, strInput,objCurrElementIndex);
		if(intTemp < 0){intReturn = -1;};
	}
	return intReturn;
};
//!Load the Comment into the Element
int	XMLReader::LoadComment(XMLElement * ptrElement,const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	std::string strValTemp;
	int intReturn = 1;
	...
	{//then OK to grab the inbetween value string
		...
		{
			XMLComment* ptrNewComment = ptrElement->AddComment();
...
int	XMLReader::LoadProcess(XMLElement * ptrElement,const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	std::string strValTemp;
	int intReturn = 1;
	size_t intStartPos, intEndPos;
...
int XMLReader::LoadProcessOpenTag(XMLElement * ptrElement,const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	int intReturn;
...
int	XMLReader::LoadProcessAttributes(XMLElement * ptrElement,const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
	//makes the assumption that the open tag has been established.
	//take the tag string minus open and close characters
	//Parse the attributes into a name,value pair vector stripping the "" or '' around the value and the = sign
...
int	XMLReader::LoadProcessValue(XMLElement * ptrElement,const std::string & strInput, XMLElementIndex & objCurrElementIndex)
{
...
long XMLReader::FindNextTagStart(const std::string & strInput,size_t intStartPos, bool & blnCloseTag)
{
...
long XMLReader::FindNextNWSLChar(const std::string & strInput,size_t intStartPos)
{
...
long XMLReader::FindNextNWSChar(const std::string & strInput,size_t intStartPos)
{
	long intCursor, lngStringSize;
	//skip through primary white space and illegal characters
	lngStringSize = strInput.size();
...
long XMLReader::FindNextWSChar(const std::string & strInput,size_t intStartPos)
{
	long intCursor,lngStringSize;
...
long XMLReader::FindNextQuote(const std::string & strInput,size_t intStartPos)
{
	long intCursor, lngStringSize;
	//cycle until white space found
	lngStringSize = strInput.size();
...
//!Find Next White Space Character
long XMLReader::FindNextChar(const std::string & strInput,size_t intStartPos, char chrFind)
{
...
bool XMLReader::IsNodeComment(const std::string & strInput,size_t intStartPos)
{
	if((intStartPos + 3) < strInput.size())
	{//then safe to check as a comment
		if((strInput[intStartPos] == '<')&&
...
bool XMLReader::IsNodeProcess(const std::string & strInput,size_t intStartPos)
{
	if((intStartPos + 1) < strInput.size())
	{//then safe to check as a process
		if((strInput[intStartPos] == '<')&&
...
bool XMLReader::IsNodeElement(const std::string & strInput,size_t intStartPos)
{
	if((intStartPos + 1) < strInput.size())
	{//then safe to check as a process
		if( (strInput[intStartPos] == '<')&&
...
void XMLReader::CleanNameString(std::string & strName)
{
	size_t i, intNumChars;
	std::string strTemp;
	intNumChars = strName.size();
	for(i = 0 ; i < intNumChars;i++)
	{
		if((strName[i] != ' ')&&(strName[i] != '='))
		{
			strTemp.push_back(strName[i]);
		};
	};
	if(intNumChars > 0)
	{
